...
export class VesselService {
  ...
  addVesselsByCompanyId(
    companyId: number,
    newVessel: Vessel,
  ): Observable<Vessel> {
    return this.http.post<Vessel>(
      `http://localhost:3000/vessels?companyId=${companyId}`,
      newVessel,
    );
  }

  deleteVesselsByCompanyId(vesselId: number): Observable<Vessel[]> {
    return this.http.delete<Vessel[]>(
      `http://localhost:3000/vessels/${vesselId}`,
    );
  }

  updateVessels(vesselId: number, updatedVessel: Vessel): Observable<Vessel> {
    return this.http.put<Vessel>(
      `http://localhost:3000/vessels/${vesselId}`,
      updatedVessel,
    );
  }
